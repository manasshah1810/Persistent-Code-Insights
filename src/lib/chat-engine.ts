...
function buildSystemPrompt(role: UserRole, dataContext: string): string {
    ...
    return `You are Persistent AI Assistant, an AI analytics assistant for the Persistent Code Insights dashboard — an AI Code Intelligence platform that tracks AI-generated vs manually-written code across engineering teams.
    ...
    private fallbackResponse(
        ...
        if (outOfScopePatterns.some(p => q.includes(p))) {
            return {
                id: generateId(),
                role: "assistant",
                content: "🚫 **Out of Scope:** I'm Persistent AI Assistant, focused exclusively on AI code intelligence analytics. This question falls outside my domain. Please ask me about your code metrics, AI tools, team performance, or dashboard navigation!",
                timestamp: new Date(),
        ...
        if (q.includes("what is") || q.includes("define") || q.includes("explain") || q.includes("meaning") || q.includes("how does") || q.includes("what does")) {
            ...
            if (q.includes("attribution")) {
                return { id: generateId(), role: "assistant", content: "💡 **Insight:** **Code Attribution** is the process of determining whether a piece of code was AI-generated or human-written. Persistent Code Insights uses a combination of VCS webhook metadata analysis, heuristic patterns, and a local Ollama LLM inference engine for attribution.", timestamp: new Date(), category: "domain" };
            }
            if (q.includes("loc") || q.includes("lines of code")) {
                return { id: generateId(), role: "assistant", content: "💡 **Insight:** **LoC (Lines of Code)** is the total number of lines of source code written. In Persistent Code Insights, this is broken down into AI-generated LoC and manually-written LoC to track developer productivity and AI adoption.", timestamp: new Date(), category: "domain" };
            }
        }
