/**
 * Professional PDF Report Generator — Persistent Code Insights
 * 
...
function addFooter(doc: jsPDF, pageNum: number, totalPages: number) {
  ...
  doc.text("Persistent Code Insights  •  Analytics Pro", PAGE.marginLeft, y);
...
function drawCoverPage(doc: jsPDF, role: UserRole, subtitle: string) {
  ...
  doc.setTextColor(...COLORS.white);
  doc.setFont("helvetica", "bold");
  doc.text("PCI", PAGE.width / 2, logoY + 11, { align: "center" } as any);

  // Brand Name
  doc.setFontSize(36);
  doc.setTextColor(...COLORS.white);
  doc.setFont("helvetica", "bold");
  doc.text("Persistent Code Insights", PAGE.width / 2, logoY + 80, { align: "center" } as any);
...
export async function exportReport(
  ...
  doc.save(`persistent-code-insights-${roleName}-report-${timestamp}.pdf`);
}
